...
async def health_check_handler(req:Request,jwtData:Optional[dict], validated_payload:None, fetched_data:List[Dict], params:Any)->ControllerResponse:
    try:
...
async def echo_handler(req:Request,jwtData:Optional[dict], validated_payload:EchoPayload, fetched_data:List[Dict], params:Any)->ControllerResponse:
    try:
